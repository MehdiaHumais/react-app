  ...
  if (shouldUseMock()) {
    ...
    return data.map(building => ({
      ...
      risers: building.risers?.map(riser => ({
        ...
        riserDetail: riser.riser_detail, // Map riser_detail
        ...
        riser_detail: riser.riser_detail
      })) || [],
  ...
  if (shouldUseMock()) {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        riser_detail: riser.riserDetail, // Insert riser_detail
      }])
      .select()
      .single();
    if (error) throw error;
...
export const findNearestRisers = (building, currentFloor) => {
  if (shouldUseMock()) {
    ...
    const classifyRiserDirection = (riserName) => {
      ...
      if (name.includes('north-east') || name.includes('northeast') || name.includes('ne')) return 'north-east';
      if (name.includes('north-west') || name.includes('northwest') || name.includes('nw')) return 'north-west';
      if (name.includes('south-east') || name.includes('southeast') || name.includes('se')) return 'south-east';
      if (name.includes('south-west') || name.includes('southwest') || name.includes('sw')) return 'south-west';
      if (name.includes('north') || name.includes('n')) return 'north';
      if (name.includes('south') || name.includes('s')) return 'south';
      if (name.includes('east') || name.includes('e')) return 'east';
      if (name.includes('west') || name.includes('w')) return 'west';
      return 'unknown';
    };

    // Add directional classification to each riser
    const risersWithDirections = risersWithFloors.map(riser => ({
      ...riser,
      direction: classifyRiserDirection(riser.number)
    }));

    // Find risers on the current floor
    const onCurrentFloor = risersWithDirections.filter(r => r.isOnCurrentFloor);
    ...
    const aboveByDirection = {};
    aboveRisers.forEach(riser => {
      const dir = riser.direction;
      if (!aboveByDirection[dir] || riser.distance < aboveByDirection[dir].distance) {
        aboveByDirection[dir] = riser;
      }
    });
    const above = Object.values(aboveByDirection);
    ...
    const belowByDirection = {};
    belowRisers.forEach(riser => {
      const dir = riser.direction;
      if (!belowByDirection[dir] || riser.distance < belowByDirection[dir].distance) {
        belowByDirection[dir] = riser;
      }
    });
...
export const findRisersOnFloor = (building, floorNumber) => {
  if (shouldUseMock()) {
    ...
    const risersOnFloor = building.risers
      .filter(riser => {
        const floors = parseFloorsCovered(riser.floorsCovered);
