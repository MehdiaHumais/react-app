...
const BottomNav = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 h-16 bg-surface flex justify-around items-center shadow-lg">
      {navItems.map((item, index) => (
        <Link
          key={index}
          to={item.path}
          className={`flex-1 flex justify-center items-center h-full text-gray-400 hover:text-white transition-colors ${item.active ? '!text-primary' : ''
            }`}
        >
          {item.icon}
        </Link>
