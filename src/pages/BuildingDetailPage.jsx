...
const BuildingDetailPage = () => {
  ...
  // Function to extract unique floors from risers' floorsCovered data
  const getFloorsFromRisers = () => {
    if (!building || !building.risers || !Array.isArray(building.risers)) {
      return [];
    }
        ...
        ranges.forEach(range => {
          if (range.includes('-')) {
            ...
            for (let i = start; i <= end; i++) {
              // Check if floor object already exists
              if (!allFloors.some(floor => floor.number === i)) {
                allFloors.push({ id: i, number: i });
              }
            }
          } else {
            ...
            if (!isNaN(floorNum)) {
              // Check if floor object already exists
              if (!allFloors.some(floor => floor.number === floorNum)) {
                allFloors.push({ id: floorNum, number: floorNum });
              }
            }
          }
        });
    ...
    return allFloors.sort((a, b) => a.number - b.number);
  };
    ...
    <div className="min-h-screen bg-[#0B181C] text-white flex flex-col">
      <div className="p-4 flex-grow relative pt-12 pb-20">
        ...
        <div className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                {building.risers.map(riser => {
                  ...
                  const riserParts = riser.number.trim().split(/\s+/);
                  ...
                  return (
                    <div key={riser.id} className="mockup-card p-3 bg-gray-800 rounded-lg">
                      ...
                      {(riser.riserDetail || riser.riser_detail) && (
                        <div className="mt-2">
                          <Button
                            onClick={() => {
                              const el = document.getElementById(`detail-${riser.id}`);
                              if (el) el.classList.toggle('hidden');
                            }}
                            className="btn-mockup-outline text-xs py-1 px-2"
                          >
                            View Riser Detail
                          </Button>
                          <div id={`detail-${riser.id}`} className="hidden mt-2 p-2 bg-gray-700 rounded text-sm whitespace-pre-wrap text-white border border-gray-600">
                            {riser.riserDetail || riser.riser_detail}
                          </div>
                        </div>
                      )}
          ...
          {building.risers && building.risers.length > 0 && (
            <div>
              <p className="text-gray-400 mb-2">Select Floor to View Risers:</p>
              <FloorSelector
                floors={getFloorsFromRisers()}  // Dynamically generate floors from risers
            ...
            <div className="space-y-2 mb-4 max-h-60 overflow-y-auto">
              {risersOnFloor.map(riser => {
                ...
                return (
                  <div
                    key={riser.id}
                    className={`mockup-card p-3 rounded-lg cursor-pointer ${selectedRiser && selectedRiser.id === riser.id
