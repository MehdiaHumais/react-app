...
const ManageBuilding = () => {
  ...
  const [activeTab, setActiveTab] = useState('buildings'); // buildings, risers, images, pdfs
  ...
  const [newRiser, setNewRiser] = useState({ name: '', direction: '', floorsCovered: '', locationDescription: '', riserDetail: '' });
  ...
  const handleAddRiser = async (e) => {
    e.preventDefault();
    console.log('handleAddRiser called', newRiser);
    if (!selectedBuildingId) {
      setError("Please select a building.");
      return;
    }
    if (!newRiser.direction) {
      setError("Please select a direction.");
      return;
    }
    if (!newRiser.floorsCovered) {
      setError("Please enter floors covered.");
      return;
    }
    setError(null);
    ...
    try {
      const newRiserObj = { number: riserNumber, floorsCovered: newRiser.floorsCovered, locationDescription: newRiser.locationDescription, riserDetail: newRiser.riserDetail };
      console.log('Adding riser:', newRiserObj);
      await addRiserToBuilding(selectedBuildingId, newRiserObj);
      console.log('Riser added successfully');
      const updatedBuildings = await getBuildings();
      setBuildings(updatedBuildings);
      const updatedBuilding = updatedBuildings.find(b => b.id === parseInt(selectedBuildingId));
      if (updatedBuilding) {
      ...
      setNewRiser({ name: '', direction: '', floorsCovered: '', locationDescription: '', riserDetail: '' });
  ...
  const startEditingRiser = (riser) => {
    ...
    const name = riserParts.length > 1 ? riserParts.slice(0, -1).join(' ') : ''; // Everything except the last part
    ...
    setNewRiser({
      ...
      riserDetail: riser.riserDetail || ''
    });
  };

  const handleUpdateRiser = async (e) => {
    e.preventDefault();
    if (!selectedBuildingId || !editingRiser || !newRiser.direction || !newRiser.floorsCovered) return;
    ...
    try {
      ...
      const updatedRiserObj = {
        ...
        riserDetail: newRiser.riserDetail
      };
    ...
    <div className="full-screen bg-dark flex flex-col">
      <div className="p-4 flex-grow overflow-y-auto relative">
        ...
        {activeTab === 'floors-risers' && (
          <div>
            {error && <div className="p-3 mb-4 bg-red-900/50 border border-red-500 text-red-100 rounded">{error}</div>}
            <select
              value={selectedBuildingId}
              onChange={(e) => setSelectedBuildingId(e.target.value)}
            ...
            {selectedBuildingId && (
              <>
                <Card className="mockup-card mb-4">
                  ...
                  <p className="text-xs text-gray-400 mb-2">Note: Multiple risers can serve the same floor.</p>
                  <p className="text-xs text-gray-400 mb-2">Most buildings have multiple risers.</p>
                  <form onSubmit={editingRiser ? handleUpdateRiser : handleAddRiser}>
                    <Input label="Riser Name" value={newRiser.name || ''} onChange={(e) => setNewRiser({ ...newRiser, name: e.target.value })} />
                    <select
                    ...
                    <Input label="Floors Covered" value={newRiser.floorsCovered} onChange={(e) => setNewRiser({ ...newRiser, floorsCovered: e.target.value })} />
                    <div className="mb-2">
                      <label className="block text-sm font-medium mb-1">Riser Detail (Optional)</label>
                      <textarea
                        value={newRiser.riserDetail || ''}
                        onChange={(e) => setNewRiser({ ...newRiser, riserDetail: e.target.value })}
                        className="w-full p-2 bg-gray-700 border border-gray-600 rounded-md text-white h-24"
                        placeholder="Enter additional details about this riser..."
                      />
                    </div>
